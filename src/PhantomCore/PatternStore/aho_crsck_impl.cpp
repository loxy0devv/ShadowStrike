...
bool AhoCorasickAutomaton::Compile() noexcept {
    ...
    // ========================================================================
    // STEP 3: VALIDATE STATE FOR NON-EMPTY AUTOMATON
    // ========================================================================
    ...
    // At this point m_nodes is non-empty, so we must have at least a root node.
    // This is a defensive check for data corruption.
    if (m_nodeCount == 0) {
        SS_LOG_ERROR(L"AhoCorasick", L"Cannot compile: internal state inconsistency");
        return false;
    }
