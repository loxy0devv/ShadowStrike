 ...
 *
 * Shared helpers for deterministic Update module unit tests.
 */
...
inline void WriteAllText(const fs::path& path, std::string_view text) {
    std::ofstream stream(path, std::ios::binary | std::ios::trunc);
    if (!stream) {
    ...
    if (!stream) {
        throw std::runtime_error("Failed to write text file");
    }
}

[[nodiscard]] inline std::vector<uint8_t> ReadAllBytes(const fs::path& path) {
...
[[nodiscard]] inline bool Contains(std::string_view text, std::string_view needle) noexcept {
