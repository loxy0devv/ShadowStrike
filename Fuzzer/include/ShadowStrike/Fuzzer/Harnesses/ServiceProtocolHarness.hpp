...
#pragma once
/**
 * @file ServiceProtocolHarness.hpp
 * @brief Fuzz harness for the embedded HTTP service protocol boundary.
 */
...
private:
    [[nodiscard]] static bool ExerciseServiceProtocolImpl(
        const uint8_t* data,
        size_t size,
        HarnessResult& result);

    [[nodiscard]] static unsigned long SEHCallServiceProtocol(
        const uint8_t* data,
        size_t size,
        HarnessResult* pResult) noexcept;
};

[[nodiscard]] int RunServiceProtocolFuzzer(
