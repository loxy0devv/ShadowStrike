...
// Forward-declare DetectionEngine so TUs that include ScanEngine.hpp do not
// need to pull in the full Detection layer headers.
namespace ShadowStrike { namespace Detection { class DetectionEngine; } }
...
struct EngineConfig {
    ...
    // Static analysis (DetectionEngine capa-style pre-scan)
    bool   enableStaticDetectionEngine = true;  ///< Run DetectionEngine::AnalyzeFile before YARA
    float  staticScoreBlockThreshold   = 75.0f; ///< aggregateScore >= this → early Infected
    float  staticScoreSuspectThreshold = 40.0f; ///< aggregateScore >= this → enrich downstream
...
public:
    ...
    /**
     * @brief Inject a DetectionEngine for Stage 0.5 static analysis.
     *
     * Call after Initialize(). If not set (or set to nullptr), the static
     * analysis stage is silently skipped — no other stages are affected.
     * Pointer is NOT owned; caller must ensure it outlives ScanEngine.
     *
     * Thread-safe: stored atomically; evaluations already in flight use
     * the previous pointer.
     */
    void SetDetectionEngine(class ShadowStrike::Detection::DetectionEngine* de) noexcept;
